...
import { replacer, reviver, translationsEqual } from '../utils/utils';
...
export class StorageService {
  ...
  getGoals(translation: string = '') {
    ...
    for(let goal of this.goalBank.goals.values()){
      if (translation == '' || translationsEqual(goal.translation, translation)){
        goals.set(goal.id, goal);
      }
    }
    console.log(`goals: ${goals.size}`);
    return goals;
  }
  ...
  getAttempts(translation: string = ''): Map<string, IResult> {
    ...
    for(let attempt of this.resultBank.results.values()){
      if (translation == '' || translationsEqual(attempt.diff.m.t, translation)){
  ...
  getLastAttempt(translation: string = ''): IResult | undefined {
    ...
    for(let attempt of attempts.values()){
      if ((lastAttempt == undefined || attempt.timestamp > lastAttempt.timestamp) && (translation == '' || translationsEqual(attempt.diff.m.t, translation))){
        lastAttempt = attempt;
      }
    }
    return lastAttempt;
